...
export default function InputGroup2({errors, touched, IsNext2, setIsNext2,isNext3, setIsNext3 }: 
    ...
    return(
        <div className={`bg-slate-80 sm:shadow-lg flex flex-col m-3 p-5 sm:h-auto absolute sm:border sm:border-orange-500 m-3 h-80 w-96 p-5 sm:h-auto justify-center	items-center	sm:items-stretch		
        ${IsNext2 ? 'opacity-100' : 'opacity-0 transition-opacity duration-500 invisible'} 
        `} >
                  <p>Veillez saisie le code de confirmation recu par sms:</p>
                   <Field 
                   type="text" 
                   name="confirm" 
                   id="confirm" 
                   className=" bg-inherit border-b-2 border-slate-500 focus:outline-none focus:border-blue-500 h-12 mt-3" 
                   placeholder="code de confirmation" 
                   />
                   <span className="block h-8 w-full text-red-700 text-xs">{touched.confirm && errors.confirm ? errors.confirm : ' '}</span>

                    <button 
                        className={`w-4/6 sm:w-full bg-blue-500  text-slate-50 p-2 hover:shadow-xl my-5 
